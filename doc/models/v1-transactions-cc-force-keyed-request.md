...
| `checkin_date` | `str` | Optional | Checkin Date - The time difference between checkin_date and checkout_date must be less than or equal to 99 days. NOTE: if checkin_date is in the future, set the advance_deposit to 1<br><br>> Required if merchant industry type is lodging.<br><br>**Constraints**: *Maximum Length*: `10`, *Pattern*: `^[\d]{4}-[\d]{2}-[\d]{2}$` |
| `checkout_date` | `str` | Optional | Checkout Date - The time difference between checkin_date and checkout_date must be less than or equal to 99 days.<br><br>> Required if merchant industry type is lodging.<br><br>**Constraints**: *Maximum Length*: `10`, *Pattern*: `^[\d]{4}-[\d]{2}-[\d]{2}$` |
...
| `standing_order` | `bool` | Optional | Standing Order |
...
| `po_number` | `str` | Optional | Purchase Order number<br><br>**Constraints**: *Maximum Length*: `64` |
...
| `secure_cryptogram` | `str` | Optional | (ECOMM) Used to supply the Digital Payment Cryptogram obtained from a Digital Secure Remote Payment (DSRP) transaction |
...
| `threedsecure` | `bool` | Optional | (ECOMM) Specify if the transaction is obtained by 3DSecure. |
| `three_ds_server_trans_id` | `str` | Optional | 3DS Server Transaction ID.  If this field is sent and 3DS authentication was done with Fortis, the 3DS fields secure_directory_server_transaction_id, secure_protocol_version, and secure_collection_indicator will be pre-populated. |
| `wallet_type` | `Any` | Optional | - |
| `clerk_id` | `str` | Optional | Clerk ID |
| `voucher_number` | `str` | Optional | Voucher Number |
| `initiation_type` | `Any` | Optional | - |
| `bill_payment` | `bool` | Optional | If transaction is a bill payment |
| `delay_charge` | `bool` | Optional | Delay Charge |
| `deferred_auth` | `bool` | Optional | Deferred Auth |
| `mini_bar` | `bool` | Optional | Mini Bar |
| `ebt_food_eligible_amount` | `int` | Optional | EBT Food Eligible Amount<br><br>**Constraints**: `>= 0`, `<= 999999999` |
| `ebt_cash_eligible_amount` | `int` | Optional | EBT Cash Eligible Amount<br><br>**Constraints**: `>= 0`, `<= 999999999` |
| `account_holder_name` | `str` | Optional | For CC, this is the 'Name (as it appears) on Card'. For ACH, this is the 'Name on Account'.<br><br>> Required for ACH transactions if account_vault_id is not provided. For CC transactions that are run through a terminal, this field may be overwritten by data acquired from the credit card track data.<br><br>**Constraints**: *Maximum Length*: `32` |
| `account_number` | `str` | Required | For CC transactions, a credit card number. For ACH transactions, a bank account number.<br><br>> String lengths are conditional, CC should be 13-19 and ACH should be 4-19. Required if account_vault_id , track_data, or micr_data is not provided.<br><br>**Constraints**: *Minimum Length*: `13`, *Maximum Length*: `19` |
...
| `pin` | `str` | Optional | Pin |
| `ksn` | `str` | Optional | KSN (Key Serial Number) |
| `auth_code` | `str` | Required | Required on force transactions and EBT voucher clear sale/refund. Ignored for all other actions.<br><br>**Constraints**: *Maximum Length*: `6` |
| `trans_id` | `str` | Optional | Trans ID<br><br>**Constraints**: *Maximum Length*: `15` |
